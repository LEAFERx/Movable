...
impl<'ctx> SymTypeTag<'ctx> {
  pub fn from_type_tag(z3_ctx: &'ctx Context, tag: &TypeTag) -> Self {
    Self {
      ast: build_type_tag_ast(z3_ctx, tag)
    }
  }

  pub fn to_type_tag(&self) -> TypeTag {
    ast_to_type_tag(&self.ast)
  }
}

impl<'ctx> SymStructTag<'ctx> {
  pub fn from_struct_tag(z3_ctx: &'ctx Context, tag: &StructTag) -> Self {
    Self {
      ast: build_struct_tag_ast(z3_ctx, tag)
    }
  }

  pub fn to_struct_tag(&self) -> StructTag {
    ast_to_struct_tag(&self.ast)
  }
}

fn build_type_tag_ast<'ctx>(z3_ctx: &'ctx Context, tag: &TypeTag) -> Datatype<'ctx> {
  ...
  let ast = match tag {
    ...
    TypeTag::Vector(vtag) => sort.variants[6].constructor.apply(&[&build_type_tag_ast(z3_ctx, vtag).into()]),
    TypeTag::Struct(stag) => sort.variants[7].constructor.apply(&[&build_struct_tag_ast(z3_ctx, stag).into()]),
  };
  ast.as_datatype().unwrap()
}

fn build_struct_tag_ast<'ctx>(z3_ctx: &'ctx Context, stag: &StructTag) -> Datatype<'ctx> {
  let sorts = make_tag_datatype_sorts(z3_ctx);
  let sort = &sorts[2];
  ...
  sort.variants[0].constructor.apply(&[
    &addr_ast.into(),
    &ast::String::from_str(z3_ctx, stag.module.as_ident_str().as_str()).unwrap().into(),
    &ast::String::from_str(z3_ctx, stag.name.as_ident_str().as_str()).unwrap().into(),
    &types_ast.into(),
  ]).as_datatype().unwrap()
}
...
fn ast_to_type_tag<'ctx>(ast: &Datatype<'ctx>) -> TypeTag {
  let sorts = make_tag_datatype_sorts(ast.get_ctx());
  let t_sort = &sorts[0];

  if test_type_variant(t_sort, ast, 0) {
    return TypeTag::Bool;
  }
  if test_type_variant(t_sort, ast, 1) {
    return TypeTag::U8;
  }
  if test_type_variant(t_sort, ast, 2) {
    return TypeTag::U64;
  }
  if test_type_variant(t_sort, ast, 3) {
    return TypeTag::U128;
  }
  if test_type_variant(t_sort, ast, 4) {
    return TypeTag::Address;
  }
  if test_type_variant(t_sort, ast, 5) {
    return TypeTag::Signer;
  }
  if test_type_variant(t_sort, ast, 6) {
    let vast = t_sort.variants[6].accessors[0].apply(&[&Dynamic::from_ast(ast)]).as_datatype().unwrap();
    let vtag = ast_to_type_tag(&vast);
    return TypeTag::Vector(Box::new(vtag));
  }
  if test_type_variant(t_sort, ast, 7) {
    let sast = t_sort.variants[6].accessors[0].apply(&[&Dynamic::from_ast(ast)]).as_datatype().unwrap();
    let stag = ast_to_struct_tag(&sast);
...
fn ast_to_struct_tag<'ctx>(ast: &Datatype<'ctx>) -> StructTag {
  let sorts = make_tag_datatype_sorts(ast.get_ctx());
  let tl_sort = &sorts[1];
  let s_sort = &sorts[2];
  ...
  for _ in 0..len {
    ...
    type_params.push(ast_to_type_tag(&ty_ast));

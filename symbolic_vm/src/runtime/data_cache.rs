...
use crate::{
  ...
  types::{
    ...
    values::{SymGlobalValue, SymGlobalValueEffect, SymValue},
  },
};
use move_core_types::{
...
impl<'ctx> SymAccountDataCache<'ctx> {
  fn new() -> Self {
    Self {
    ...
    Self {
      data_map: self.data_map
        .iter().map(|(key, value)| {
          ...
          let cloned_value = (value.0.clone(), value.1.clone_for_symbolic_state_fork());
        ...
        Ok(Some(blob)) => {
          let val = match Value::simple_deserialize(&blob, &ty_layout) {
            Some(val) => {
              ...
              SymValue::from_deserialized_value(self.z3_ctx, val, &ty_tag)?
            },
            None => {

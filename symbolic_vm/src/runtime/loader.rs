...
use vm::{
  ...
  file_format::{
    ...
    StructDefInstantiationIndex, StructDefinition, StructDefinitionIndex, StructFieldInformation,
    TableIndex,
  },
  IndexKind,
};
...
impl ModuleCache {
  ...
  fn add_module(&mut self, module: &CompiledModule, log_context: &impl LogContext) -> VMResult<()> {
    ...
    self
      .load_field_types(module, starting_idx, log_context)
      .map_err(|err| {
...
impl Loader {
  ...
  pub(crate) fn load_script<'ctx>(
    ...
    self
      .verify_ty_args(main.type_parameters(), &type_arguments)
  ...
  fn load_type<'ctx>(
    ...
    Ok(match type_tag {
      ...
      TypeTag::Vector(tt) => Type::Vector(Box::new(self.load_type(tt, data_store, log_context)?)),
      TypeTag::Struct(struct_tag) => {
        ...
        if struct_type.type_parameters.is_empty() && struct_tag.type_params.is_empty() {
          ...
          self
            .verify_ty_args(&struct_type.type_parameters, &type_params)
            .map_err(|e| e.finish(Location::Undefined))?;
  ...
  fn load_module<'ctx>(
    ...
    fn deserialize_and_verify_module<'ctx>(
      ...
      loader.verify_module_expect_no_missing_dependencies(&module, data_store, log_context)?;
      Ok(module)
    }
  ...
  fn abilities(&self, ty: &Type) -> PartialVMResult<AbilitySet> {
    match ty {
      Type::Bool | Type::U8 | Type::U64 | Type::U128 | Type::Address => Ok(AbilitySet::PRIMITIVES),

...
use vm::{
  ...
  file_format::{
    ...
    Kind, Signature, SignatureToken, StructDefInstantiationIndex, StructDefinition,
...
impl ModuleCache {
  ...
  fn load_fields(&mut self, module: &VerifiedModule, starting_idx: usize) -> VMResult<()> {
      ...
      .zip(&mut self.structs[starting_idx..])
    {
      match Arc::get_mut(arc_struct_type) {
        None => {
          return Err(
            VMStatus::new(StatusCode::INVALID_CODE_CACHE)
  ...
  fn resolve_function_handle(&self, func_name: &IdentStr, module_id: &ModuleId) -> VMResult<usize> {
    ...
    Err(
      VMStatus::new(StatusCode::UNREACHABLE).with_message("Missing function in cache".to_string()),
    )
  }
  ...
  fn deserialize_and_verify_script(
    ...
    let script = match CompiledScript::deserialize(script) {
      ...
      Err(err) => {
        ...
        let error =
          vm_error(Location::default(), StatusCode::CODE_DESERIALIZATION_ERROR).append(err);
        return Err(error);
      }
    };
...
impl Function {
  ...
  pub(crate) fn get_native(&self) -> VMResult<NativeFunction> {
    self.native.ok_or_else(|| {
      VMStatus::new(StatusCode::UNREACHABLE).with_message("Missing Native Function".to_string())
    })
  }
}

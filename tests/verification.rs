...
fn make_struct_type(address: &str, module: &str, name: &str, type_params: Vec<TypeTag>) -> TypeTag {
  let tag = StructTag {
    ...
    module: Identifier::from(IdentStr::new(module).unwrap()),
    name: Identifier::from(IdentStr::new(name).unwrap()),
...
fn alias_bar2() {
  ...
  fn bar2_aborts_if<'ctx>(z3_ctx: &'ctx Context, ty_ctx: &TypeContext<'ctx>, a: &[SymValue<'ctx>], om: &SymMemory<'ctx>, _m: &SymMemory<'ctx>) -> SymBool<'ctx> {
    // TODO: bad type conversions and clones
    // TODO: figure it out
    let addr = signer_addr(ty_ctx, a[0].copy_value().unwrap());
    let ty = make_struct_type("0x123", "M", "U", vec![TypeTag::U64]);
...
fn alias_bar1() {
  ...
  fn bar_aborts_if<'ctx>(z3_ctx: &'ctx Context, ty_ctx: &TypeContext<'ctx>, a: &[SymValue<'ctx>], om: &SymMemory<'ctx>, _m: &SymMemory<'ctx>) -> SymBool<'ctx> {
    // TODO: bad type conversions and clones
    // TODO: figure it out
    let account = signer_addr(ty_ctx, a[0].copy_value().unwrap());
